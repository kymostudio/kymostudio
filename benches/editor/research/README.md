...
| Date | Article | Gist |
|---|---|---|
| 2026-06-12 (5) | [Round 5: a Rust mermaid after all — one grammar, linker-sliced from merman](2026-06-12-merman-flowchart-slice.md) | Flowcharts render through `kymo-mermaid`: merman pinned by rev, the other 22 grammars LTO-stripped by calling the flowchart pipeline directly (9.8 MB → 1.5 MB raw, ~473 KB brotli — 38% under mermaid.js) with mermaid-11 parity. Fresh path sheds ~330 KB / ~350 ms; the 900 ms race window is now the headline target. |
