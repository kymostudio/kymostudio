...
for (const ds of ["merman","mermaid-cypress","mermaid-to-svg"]) {
  ...
  for (const g of order) {
    ...
    for (const fn of sample) {
      ...
      n++; mjsS+=recall(vis,rast); kS+=recall(vis,kt); mS+=recall(vis,mt);
    }
    if(n) out[ds].push({g, n, mjs:+(100*mjsS/n).toFixed(0), kymo:+(100*kS/n).toFixed(0), merman:+(100*mS/n).toFixed(0), own:!!KYMO[g]});
  }
}
if(browser) await browser.close();
for (const ds of ["merman","mermaid-cypress","mermaid-to-svg"]) {
  console.log("\n### "+ds);
  console.log("diagram        | n   | mermaid.js | kymo  | merman | kymo-engine");
  console.log("---------------|-----|-----------|-------|--------|------------");
  for (const r of out[ds]) console.log(`${r.g.padEnd(14)} | ${String(r.n).padStart(3)} | ${String(r.mjs).padStart(8)}% | ${String(r.kymo).padStart(4)}% | ${String(r.merman).padStart(5)}% | ${r.own?"own":""}`);
}
