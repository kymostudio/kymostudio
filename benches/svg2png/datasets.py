#!/usr/bin/env python3
"""Accuracy dataset — vendored resvg test-suite SVGs + Chrome ground truth.

The fidelity bench (`quality.py`) scores engines against *kymo's own* output, so
it answers "who reproduces kymo", not "who is correct". This module backs the
**accuracy** bench (`accuracy.py`): a vendored, MIT-licensed subset of the
[resvg test suite](https://github.com/linebender/resvg-test-suite) — the de-facto
standard SVG conformance corpus — each paired with a **headless-Chrome reference
...
Scope: self-contained, text-free SVGs across 6 categories (shapes, painting,
paint-servers, structure, masking, filters). `text`/`image` are excluded — they
depend on bundled fonts / external resources that confound *rasterizer* accuracy
and can't be resolved through our string-based engine API. See `PROVENANCE.md`.

`normalize_svg` pins every SVG to its viewBox pixel size (the suite ships
`viewBox`-only SVGs), so Chrome and all engines rasterize the *same* canvas and
...
DATASET_DIR = HERE / "datasets" / "resvg-suite"
...
def normalize_svg(svg: str, default: int = 200) -> tuple[str, int, int]:
    ...
    The suite's SVGs carry a `viewBox` but no `width`/`height`, so every renderer
    would otherwise apply its own intrinsic-size default. We strip any existing
    width/height and inject the viewBox extent, so the canvas is identical for
    Chrome and every engine.
    """
