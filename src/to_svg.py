...
.edge-path {
  ...
  filter: drop-shadow(0 1px 0.5px rgba(15, 23, 42, 0.08));
}
...
def render_region_rect(r: Region) -> str:
    ...
    if getattr(r, "style", "outer") == "inner":
        rect_cls += " region-rect--inner"

    icon_key = getattr(r, "icon", None)
    icon_g = (
        f'<g transform="translate({x + 18}, {y})">{ICONS[icon_key]}</g>\n'
        if icon_key else ""
    )
...
def render_region_label(r: Region) -> str:
    ...
    if getattr(r, "icon", None):
        ...
        # (outer → above, inner → inside — inner is usually nested so its
        # own label needs to sit inside its own rect, not above where it
        # could collide with the parent outer region's label).
        pos = getattr(r, "label_position", None)
        if pos is None:
            pos = "inside" if getattr(r, "style", "outer") == "inner" else "above"
...
def render_edge(e: Edge, d: Diagram) -> str:
    ...
    label_svg = (f'<text class="{lcls}" x="{lx}" y="{ly}">{_x(e.label)}</text>'
                 if e.label else "")
    return (
        f'<path class="{cls}"{dash_attr} d="{path}" marker-end="{marker}"/>\n'
        f'{label_svg}'
    )
