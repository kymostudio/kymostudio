...
/// Render a Mermaid `classDiagram` -> SVG (multi-compartment, text-based).
#[wasm_bindgen(js_name = mermaidClassToSvg)]
pub fn mermaid_class_to_svg(src: &str) -> Result<String, JsError> {
...
/// Render a Mermaid `block` / `block-beta` diagram -> SVG (text-based).
#[wasm_bindgen(js_name = mermaidBlockToSvg)]
pub fn mermaid_block_to_svg(src: &str) -> Result<String, JsError> {
