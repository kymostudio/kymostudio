...
use kymo_font_loader::FontSet;

fn sfnt_collection_index(id: FontId) -> u32 {
    match id {
        FontId::EmojiFallback => kymo_unicode_font::emoji_font_face_index().unwrap_or(0),
        FontId::CjkRegular => kymo_unicode_font::unicode_font_face_index().unwrap_or(0),
        FontId::CjkFallback => kymo_unicode_font::fallback_font_face_index().unwrap_or(0),
        _ => 0,
    }
}
...
/// Vector path or color-emoji raster (`sbix` PNG as `data:image/png`), matching `kymo-render::render_glyph`.
#[derive(Debug)]
...
/// Same geometry as `kymo-render`: SVG user space, y downward. Emoji uses bitmap **before** outline
...
pub(crate) fn standalone_glyph(
    ...
    let ch = kymo_font::katex_ttf_glyph_char(font_id, char_code);
...
fn try_emoji_png_data_url(px: f32, py: f32, em: f32, ch: char) -> Option<StandaloneGlyph> {
    ...
    let strike = kymo_unicode_font::emoji_png_raster_for_char(ch, request_em)?;
    ...
    // Match `kymo-render::try_blit_raster_glyph`: `y` is the bitmap bottom in y-up strike space;
    ...
    let axis = kymo_font::get_global_metrics(0).axis_height as f32;
...
fn outline_to_d(
    ...
    let curves = kymo_font_loader::outline_cache::get_or_compute_outline(
