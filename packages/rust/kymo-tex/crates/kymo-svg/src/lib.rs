...
//!
//! **Glyphs (default):** each [`DisplayItem::GlyphPath`](kymo_types::display_item::DisplayItem::GlyphPath)
...
pub fn render_to_svg(list: &DisplayList, opts: &SvgOptions) -> String {
    ...
    let prerendered_glyphs: Option<Vec<Option<standalone::StandaloneGlyph>>> = {
        if load_fonts {
            if let Ok(fonts) = kymo_font_loader::load_fonts_for_items(&opts.font_dir, &list.items)
            {
...
mod tests {
    ...
    fn embedded_emoji_image_uses_color_alpha_as_opacity() {
        ...
        let ch = '😀';
        if kymo_unicode_font::emoji_png_raster_for_char(ch, 10.0).is_none() {
            return;
        }
