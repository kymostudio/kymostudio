use kymo_tex_lexer::token::{SourceLocation, Token};
...
impl<'a> Parser<'a> {
    ...
    fn parse_symbol_inner(&mut self) -> ParseResult<Option<ParseNode>> {
        ...
        let font_mode = match self.mode {
            Mode::Math => kymo_tex_font::symbols::Mode::Math,
            Mode::Text => kymo_tex_font::symbols::Mode::Text,
        };
        ...
        if let Some(sym_info) = kymo_tex_font::symbols::get_symbol(&text, font_mode) {
            ...
            let node = if group.is_atom() {
                let family = match group {
                    kymo_tex_font::symbols::Group::Bin => AtomFamily::Bin,
                    kymo_tex_font::symbols::Group::Close => AtomFamily::Close,
                    kymo_tex_font::symbols::Group::Inner => AtomFamily::Inner,
                    kymo_tex_font::symbols::Group::Open => AtomFamily::Open,
                    kymo_tex_font::symbols::Group::Punct => AtomFamily::Punct,
                    kymo_tex_font::symbols::Group::Rel => AtomFamily::Rel,
                    _ => unreachable!(),
                };
                ParseNode::Atom {
                    mode: self.mode,
                ...
                match group {
                    kymo_tex_font::symbols::Group::MathOrd => ParseNode::MathOrd {
                        mode: self.mode,
                        text: text.clone(),
                        loc,
                    },
                    kymo_tex_font::symbols::Group::TextOrd => ParseNode::TextOrd {
                        mode: self.mode,
                        text: text.clone(),
                        loc,
                    },
                    kymo_tex_font::symbols::Group::OpToken => ParseNode::OpToken {
                        mode: self.mode,
                        text: text.clone(),
                        loc,
                    },
                    kymo_tex_font::symbols::Group::AccentToken => ParseNode::AccentToken {
                        mode: self.mode,
                        text: text.clone(),
                        loc,
                    },
                    kymo_tex_font::symbols::Group::Spacing => ParseNode::SpacingNode {
                        mode: self.mode,
                        text: text.clone(),
                        loc,
                    },
    ...
    fn try_parse_unicode_accent(
        ...
        let mut node = if base_str.chars().count() == 1 {
            ...
            if let Some(sym) = kymo_tex_font::symbols::get_symbol(&base_str, font_mode) {
                match sym.group {
                    kymo_tex_font::symbols::Group::TextOrd => ParseNode::TextOrd {
                        mode: self.mode,
                        text: base_str.clone(),
