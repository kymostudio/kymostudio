//! Extract color-emoji bitmap strikes (`sbix`, `CBDT`, …) for PNG/SVG/PDF parity with `kymo-render`.
...
///
/// `glyph_em_px` is the requested em size in pixels (same convention as `kymo-render` pixmap).
