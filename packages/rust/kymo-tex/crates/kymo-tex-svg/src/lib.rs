//! Serialize a [`DisplayList`](kymo_tex_types::display_item::DisplayList) to SVG.
//!
...
//!
//! **Glyphs (default):** each [`DisplayItem::GlyphPath`](kymo_tex_types::display_item::DisplayItem::GlyphPath)
...
//! `kymo-katex-fonts` crate. Color emoji prefer embedded PNG strikes and fall back to outline
...
pub struct SvgOptions {
    /// User units per em (matches `kymo_render::RenderOptions::font_size` at DPR 1).
    pub font_size: f64,
...
mod tests {
    ...
    fn embedded_emoji_image_uses_color_alpha_as_opacity() {
        ...
        let ch = '😀';
        if kymo_tex_unicode_font::emoji_png_raster_for_char(ch, 10.0).is_none() {
            return;
        }
