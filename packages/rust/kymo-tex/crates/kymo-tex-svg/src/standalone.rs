...
use kymo_tex_font_loader::FontSet;

fn sfnt_collection_index(id: FontId) -> u32 {
    match id {
        FontId::EmojiFallback => kymo_tex_unicode_font::emoji_font_face_index().unwrap_or(0),
        FontId::CjkRegular => kymo_tex_unicode_font::unicode_font_face_index().unwrap_or(0),
        FontId::CjkFallback => kymo_tex_unicode_font::fallback_font_face_index().unwrap_or(0),
        _ => 0,
    }
}
...
pub(crate) fn standalone_glyph(
    ...
    let ch = kymo_tex_font::katex_ttf_glyph_char(font_id, char_code);
...
fn try_emoji_png_data_url(px: f32, py: f32, em: f32, ch: char) -> Option<StandaloneGlyph> {
    ...
    let strike = kymo_tex_unicode_font::emoji_png_raster_for_char(ch, request_em)?;
    ...
    let axis = kymo_tex_font::get_global_metrics(0).axis_height as f32;
