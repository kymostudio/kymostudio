...
//! Moved out of `kymo-graph` so all graph-layout algorithms live in one crate.
//! Node sizing + text metrics come from `kymo_graph::metrics`.

use std::collections::{HashMap, HashSet};

use kymo_graph::flowchart::{Direction, FlowEdge, Flowchart};
use kymo_graph::metrics::node_size;
use kymo_graph::model::{py_round, Component, Diagram, Edge, Region};
