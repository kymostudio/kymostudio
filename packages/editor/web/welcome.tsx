...
export function WelcomeView({ onNew, onOpenFile, onTemplate }: {
  ...
  const startBlock = (
    <section className="wel-block" key="start">
      ...
      <button className="wel-link" data-testid="wel-open" onClick={() => fileRef.current?.click()}><FolderOpen size={17} strokeWidth={2} />Open file…</button>
    ...
    <section className="wel-block" key="recent">
      ...
      {!claims ? (
        <p className="wel-note">
          ...
          <button className="wel-inline-link" data-testid="wel-signin" onClick={() => (window as any).google?.accounts?.id?.prompt?.()}>Sign in</button>{" "}
