...
// Sign-in page: auth-walled routes redirect here with ?next=<path>; once GIS
// hands back a token we bounce straight to it (default: the editor home).
export default function LoginPage() {
  const { claims } = useAuth();
  const [params] = useSearchParams();
  const navigate = useNavigate();
  const raw = params.get("next") || "/";
  const next = raw.startsWith("/") && !raw.startsWith("//") ? raw : "/"; // same-app paths only
