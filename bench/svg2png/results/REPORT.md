...
> Generated by `bench/svg2png/run.py`. **Offline bench** — re-run with
> `cd bench && uv run python svg2png/run.py` to refresh. It rasterizes a corpus
...
|---|---|---|---|---|---|---|---|---|---|
| kymo | `resvg (kymostudio-core 0.3.6)` | 30/30 | 30/30 | — | — | 11.748 | 58.6 | ×1.0 | **reference** |
| resvg-py | `resvg (0.3.2)` | 30/30 | 30/30 | 0.0 | 0.0% | 19.247 | 32.6 | ×0.56 | **pixel-identical** |
| pyvips | `librsvg (via libvips 8.15.1)` | 30/30 | 30/30 | 0.24 | 4.07% | 15.67 | 18.1 | ×0.31 | **high fidelity** |
| cairosvg | `Cairo (2.9.0)` | 30/30 | 30/30 | 11.24 | 99.73% | 7.252 | 109.0 | ×1.86 | **low fidelity** |
...
- **Corpus** — `bench/svg2png/corpus.py` renders every `samples/*.kymo` and
...
- **Quality** — `bench/svg2png/quality.py` decodes each PNG (Pillow), composites
  over white, and compares to the kymo reference: render success, dimension
  match, mean |Δ|, and % pixels differing.
- **Performance** — `bench/svg2png/perf.py` times each engine (median of
...
- Timing is host-specific (`Linux-6.8.0-117-generic-x86_64-with-glibc2.39`, Python 3.13.13,
  4 CPU, reps=7, 2026-06-06T11:50:47+00:00). Compare runs
  on the same machine.
