...
**Next:** **Phase 6 — Sticky** (`FR-J-06`, 5 SP): a click-to-place resizable `note` shape with an
editable plain-text label (click to place; double-click to edit). Same freeform-layer contract
(`meta.kymo == null`, persist-only). The `note` tool reverts to `select` on commit (click-to-place,
unlike the multi-stroke pen). Continues un-deferring `TC-06` (`RK-EN-03`: one tool per phase).
