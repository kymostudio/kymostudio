...
| RK-EN-05 | Engine bundle not as small as hoped after removal | Low | Low | Measure each phase (`NFR-J-02`); tree-shake; target ≤ ~50 KB gzip | Open |
...
| **RK-J-01** | tldraw-removal regresses a `TC-01..19` case that only tldraw covered | Med | High | Removed P3 with `TC-01..05/07..19` green on the engine; **`TC-06`'s freeform clause explicitly deferred to P5–7** (the engine has no freeform tools yet — a planned gap, not a silent regression). Re-pin via `git revert` if needed. | **Closed (TC-06 deferred)** |
...
**Next:** **Phase 4 — Footprint** (`NFR-J-01/02`, 5 SP): the bundle measurement is effectively done
(107 KB gzip); remaining is the **culling + per-record reactivity** perf pass (`RK-EN-04` / the
`BENCH-ENGINE-001` lever) — viewport culling + per-shape atoms so drag-at-high-N and huge mounts stay smooth.
