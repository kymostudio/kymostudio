...
The opinionated prior-art comparison — at-a-glance matrix, headline tradeoffs, a per-category scoring of Flowable against kymo, and open questions for kymo — lives in [`flowable.comparision.md`](z.flowable.comparision.md). It is kept separate so it can evolve at a different cadence than this factual reference (kymo changes alone are enough to invalidate it, even when upstream Flowable has not moved).
