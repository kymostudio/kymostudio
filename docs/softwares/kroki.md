...
| Related Documents | [REF-KROKI-CMP-001](./kroki.comparision.md), [REF-D2-001](d2.md), [REF-BPMNIO-001](bpmn-io.md), [KYMO-DSL-001](../formats/kymo-dsl/README.md), [BPD-DGM-001](../diagrams/best-practices.md) |
...
Read this first if you need ground truth on how Kroki actually behaves; the opinionated comparison against kymo — including the per-category scoring — lives in [REF-KROKI-CMP-001](kroki.comparision.md).
...
Two entries matter for kymo specifically: **D2** (a documented kymo peer — see [REF-D2-001](d2.md)) and **BPMN** + **Excalidraw**, both of which kymo already touches (BPMN import; an Excalidraw exporter). Kroki therefore *bundles several of kymo's neighbours* under one roof — that overlap is the subject of the comparison.
...
1. **Landscape map.** Kroki bundles a kymo peer (**D2**, see [REF-D2-001](d2.md)) and two kymo interop targets (**BPMN**, **Excalidraw**, see [REF-BPMNIO-001](bpmn-io.md)). Its catalogue is a useful census of what "text → diagram" tools exist and how they are packaged.
...
The opinionated version of both threads — with the at-a-glance matrix, headline tradeoffs, a per-category scoring against kymo, and open questions — lives in [REF-KROKI-CMP-001](kroki.comparision.md). It is kept separate so it can evolve at a different cadence than this factual reference (kymo changes alone are enough to invalidate it, even when upstream Kroki has not moved).
