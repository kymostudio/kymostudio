...
| Parent Reference  | [REF-KROKI-001](kroki.md)                                      |
| Related Documents | [REF-D2-CMP-001](d2.comparision.md), [KYMO-DSL-001](../formats/kymo-dsl/README.md), [BPD-DGM-001](../diagrams/best-practices.md) |

This document isolates the **prior-art comparison** between [Kroki](https://kroki.io/) and kymo. The factual reference (architecture, API encoding, supported types, output formats, deployment) lives in [REF-KROKI-001](kroki.md); read that first if you need ground truth on how Kroki actually behaves.
...
Kroki's catalogue is a useful map of the "text → diagram" world, and kymo lives inside it: Kroki bundles **D2** (kymo's closest documented peer — see [REF-D2-CMP-001](d2.comparision.md)) and renders **BPMN** and **Excalidraw**, both of which kymo already touches (BPMN import; an Excalidraw exporter; see [REF-BPMNIO-001](bpmn-io.md)). This means the question "where does kymo fit?" can be answered concretely: it would be *one more diagram type* alongside D2 and Mermaid in the same gateway — a focused, opinionated architecture-diagram renderer where the others are general-purpose.
...
The matrix in §1 says *what* differs; this section grades *how well* each tool handles each dimension, using the same five categories and 15 criteria as [REF-D2-CMP-001](d2.comparision.md) so the corpus stays comparable. The per-category totals roll up to an overall in §3.6.
