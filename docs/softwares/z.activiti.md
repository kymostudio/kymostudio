...
- **2003** — jBPM appears (the first OSS BPM; see [`jbpm.md`](./z.jbpm.md)).
...
The opinionated prior-art comparison — at-a-glance matrix, headline tradeoffs, a per-category scoring of Activiti against kymo, and open questions for kymo — lives in [`activiti.comparision.md`](z.activiti.comparision.md). It is kept separate so it can evolve at a different cadence than this factual reference (kymo changes alone are enough to invalidate it, even when upstream Activiti has not moved).
