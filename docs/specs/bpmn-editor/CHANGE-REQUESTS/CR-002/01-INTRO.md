...
> the `bpmn-editor` spec (`BPMN-EDITOR-001`), mirroring the parent layout (`01-INTRO`→`05-PLAN`). This
...
lane's tasks) by fill/stroke color. `FEAT-BPMN-EDITOR-001 §6` **deferred** it from v1 because it cannot
...
| Clause | Doc | Change |
|--------|-----|--------|
| §3.2 context pad / §6 | `FEAT-BPMN-EDITOR-001` | Add the Set-color action + color round-trip; drop color from out-of-scope |
