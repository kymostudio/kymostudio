...
| **TC-KE-10** | Socket loss behaviour | Kill the WS (devtools/offline) → editing continues locally; navigating to the room again (or token refresh) re-establishes the socket. **No "Offline" pill is surfaced** — a sustained disconnect is `console.debug`'d only (a transient drop, e.g. a worker redeploy, must not alarm the user). *No timed auto-reconnect is expected* (risk R10). | FR-KE-06 |
