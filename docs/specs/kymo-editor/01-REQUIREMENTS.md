...
| Status            | Implemented |
| Owner             | `diagrams/` project |
| Related Documents | `DESIGN-KEDITOR-001` (how), `TEST-KEDITOR-001` (V&V), `PLAN-KEDITOR-001` (plan), `FEAT-KRENDER-001` / `FEAT-KSHARE-001` / `FEAT-KLIVE-001` / `FEAT-KLIBRARY-001` / `FEAT-KEMCP-001` / `FEAT-KHOME-001` / `FEAT-KAI-001` (the seven modules — see §B.7), `FEAT-FLOWCHART-001` (the native DSL), `FEAT-KYMOJSON-001` (the engine intermediate), `FEAT-KMCP-001` (the sibling *local* npx MCP server), `FEAT-CANVAS-001` / `FEAT-STUDIO-001` (sibling canvas editors), `FEAT-KRAPI-001` (the **render.kymo.studio** Worker the editor now delegates non-kymo rendering to), `REF-KROKI-001` (the upstream render gateway, now a fallback behind `FEAT-KRAPI-001`), `RES-MCP-001` (MCP landscape) |
...
- `modules/` — the seven as-built sub-module doc-sets (§B.7): `editor-render`, `editor-share`, `editor-live`, `editor-library`, `editor-mcp`, `editor-home`, `editor-ai`.
...
- **umbrella / module** — `kymo-editor` is the umbrella (system); `editor-render`/`editor-share`/`editor-live`/`editor-library`/`editor-mcp`/`editor-home`/`editor-ai` are its modules (system elements) under `modules/`.
- **UserChannel** — a **per-user** Durable Object (keyed by the OAuth `email`) backing the Connect AI control plane: presence, window targeting, the `ui_status` feed, open/close/open-project control, the `wait_for_user_message` reverse-channel inbox, and the UI-simulation triggers. Distinct from the per-**diagram** `EditorRoom`. Owned by `editor-ai` (`FEAT-KAI-001`).
- **Connect AI panel** — the right-docked AI sidebar (`web/connectai.tsx`), toggled by the ✨ activity-bar button: Chat (live feed + composer), Connection (presence + window pin + session id), Setup (connect instructions). Owned by `editor-ai` (`FEAT-KAI-001`).
...
kymo-editor (FEAT-KEDITOR-001) → UMBRELLA — the shipped editor.kymo.studio product
        ...
        └── editor-ai       (FEAT-KAI-001)      → Connect AI panel & control plane [as-built]
```

Dependency direction: `editor-library` and `editor-mcp` build on the `editor-live` spine; `editor-render` and `editor-share` work signed-out, independent of it; `editor-home` (the landing surface) sits over `editor-library`/`editor-render`/`editor-live`; `editor-ai` (the AI-connection panel & control plane) sits over `editor-mcp` (and through it, `editor-live`).
...
| `editor-ai` | `SN-AI-01..05` (module-native, elaborate `SN-KE-03/04`) | `FR-AI-01..10` + `US-AI-01..05` (module-native; UI/UX user-story layer) | `NFR-AI-01..02` (module-native) |
...
**v0.6 module addition:** the **Connect AI** experience — the editor-side AI panel + a per-user **control plane** — is specified in the new **`editor-ai`** module (`FEAT-KAI-001`): the **`UserChannel`** Durable Object (presence, window targeting, the `ui_status` Live Activity feed, the `wait_for_user_message` reverse channel + durable inbox + `listening` composer-gating, UI-simulation of `new_project`/`delete_project`, live project-list sync) and the **Connect AI panel** (Chat/Connection/Setup, chat bubbles, Simulate-UI settings gear, Manage-projects modal, connect instructions incl. Claude Code). It **builds on `editor-mcp`** (`FR-MC-05` `ui_*` family + `/userws`) and carries `SN-AI-01..05` / `FR-AI-01..10` / `NFR-AI-01..02` + a `US-AI-01..05` user-story layer. See §C.10 for the catalogue row.
...
| Connect AI (panel + control plane) | `FR-AI-01..10` (`FEAT-KAI-001`) | The right-docked **Connect AI** panel (Chat/Connection/Setup) over a per-user **`UserChannel`**: live activity feed (`ui_status`), window sessions/targeting (`ui_list_sessions`/`ui_switch_session`), web→agent reverse channel (`wait_for_user_message` + durable inbox + `listening` composer-gating), UI-simulation of `new_project`/`delete_project`, live project-list sync (no reload), and connect instructions (incl. Claude Code). Builds on `editor-mcp` (`FR-MC-05`). UI/UX-heavy → carries `US-AI-01..05`. |
...
| 0.6     | 2026-06-20 | Vũ Anh | **Register the `editor-ai` (Connect AI) module.** Added a **seventh** module `editor-ai` (`FEAT-KAI-001`, `DESIGN-KAI-001`) — the editor-side AI panel + a per-user **`UserChannel`** control plane (live activity feed `ui_status`; window sessions/targeting `ui_list_sessions`/`ui_switch_session`; web→agent reverse channel `wait_for_user_message` + durable inbox + `listening` composer-gating; `new_project`/`delete_project` `simulate` UI-animation; live project-list sync; connect instructions incl. Claude Code), shipped over PRs ~#514→#598. It **builds on `editor-mcp`** (`FR-MC-05` `ui_*` family + `/userws`) and carries `SN-AI-01..05` / `FR-AI-01..10` / `NFR-AI-01..02` + `US-AI-01..05`. Updated §B.2/§B.6/§B.7 (module tree six→seven, re-homing map, dependency note, glossary: UserChannel + Connect AI panel) and added the Connect-AI row to §C.10. Docs-only — no code/behaviour change. The session's keyboard-shortcuts + activity-bar Settings menu are **out of scope** here (global editor UX → `editor-render` / a separate CR). |
