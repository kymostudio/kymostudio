...
### C.1 Functional requirements — MCP channel (`FR-MC-01..04`)

| ID | Requirement | Source need |
|----|-------------|-------------|
| **FR-MC-01** | The Worker SHALL expose **per-user diagram tools** over MCP — `new_diagram(title?, source?, kind?, project?)`, `list_diagrams(project?)`, `edit_diagram(source?, title?, id?, kind?)`, `get_diagram(id?)`, `delete_diagram(id)`, `open_diagram(id?)` — operating only on the authenticated user's diagrams; `edit_diagram`/`get_diagram`/`open_diagram` default to the user's **most recent** diagram when `id` is omitted. Content writes broadcast live to the user's open tabs and report the live-tab count; create/edit responses include the `?d=` URL. `open_diagram` live-switches which diagram the user's open tabs show (without altering content) and reports the switched-tab count. *(v0.2: `delete_diagram` is a **soft** delete — it moves the diagram to Trash, recoverable for 30 days, the same `destroyDiagram` path the browser uses — `FR-LV-09`. v0.3: `new_diagram`/`list_diagrams` take an optional `project` arg — see `FR-MC-04`.)* | SN-MC-01 |
...
4. `new_project` returns an id + name; `list_projects` lists them with the default flagged; `new_diagram(project)` files the diagram under it and `list_diagrams(project)` returns only that project's diagrams; `move_diagram(id, project)` reassigns it; `delete_project` soft-cascades its contents to Trash and refuses to remove the user's only project; `open_project` switches the active project an open tab is scoped to and reports the switched-tab count. A `project` ref resolves by id or name; another user's diagram/project id is refused ("isn't yours" / "not found").
