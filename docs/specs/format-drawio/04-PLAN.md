...
| Owner        | `diagrams/` project                                |
| Related      | FEAT-DRAWIO-001, FEAT-DRAWIO-001, DESIGN-DRAWIO-001, TEST-DRAWIO-001, PLAN-DRAWIO-SVG-001 |

Concept: FEAT-DRAWIO-001. Requirements (family FR/NFR): FEAT-DRAWIO-001. Design:
...
`drawio-raster` are registered as **Proposed** in the `FEAT-DRAWIO-001` §3 registry.
...
`FEAT-DRAWIO-001` registry; keep family FR/NFR + the TEST traceability consistent; increment
