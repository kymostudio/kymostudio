...
| Owner             | `diagrams/` project |
| Entry gate        | **CR-001 approved** (`FEAT-BPMN-EDITOR-CR-001 §5`) and the v1 spec baselined. No code begins before both. |
...
`FEAT-BPMN-EDITOR-CR-001` status = Closed; register row = Closed.
...
the parent clauses named in `FEAT-BPMN-EDITOR-CR-001 §3` are reconciled, each parent doc's version +
