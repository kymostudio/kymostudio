...
| Owner        | `diagrams/` project                                |
| Related      | FEAT-BPMN-PARSER-001, DESIGN-BPMN-PARSER-001, TEST-BPMN-PARSER-001 |

Requirements (FR/NFR referenced below; product context + stakeholder needs):
...
Changes to the baselined spec (`docs/specs/format-bpmn/modules/parser/`) are raised, assessed, and logged in
...
Version-controlled at `docs/specs/format-bpmn/modules/parser/04-PLAN.md`; authoritative source is the
...
| 2026-05-24 | P3 — Docs & release | Authored the feature doc set `docs/specs/format-bpmn/modules/parser/` (INTRO/REQ/DSN/TST/PLAN, `*-BPMN-PARSER-001`) — the symmetric counterpart to the `bpmn-export` set — and added a back-reference from BPMN-MAP-001. Doc set marked **Released**. | **Done** — feature complete (20/20 pts); citations resolve; structure mirrors `bpmn-export` 1:1. |
