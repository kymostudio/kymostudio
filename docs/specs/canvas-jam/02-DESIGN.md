...
- **Perf — as-built (P4):** the coarse single-epoch reactivity (`DESIGN-ENGINE-001` §5.4) was **refined
  ...
  shapes (`TEST-ENGINE-001` §4.4: 600-node drag holds 60 fps median vs 30 fps parent-wide). The
...
- **Culling deferred (was `DESIGN-ENGINE-001` §8.2 on/off comparison).** Decided **against** viewport
  culling: `TEST-ENGINE-001` §6 shows the engine beats tldraw at scale *because* it does 0 re-renders on
