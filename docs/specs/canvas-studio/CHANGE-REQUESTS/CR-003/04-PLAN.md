...
| Owner             | `diagrams/` project |
| Entry gate        | **CR-003 approved** (`FEAT-STUDIO-003 §5`). No code begins before approval. |
...
- **P3:** edit the parent baseline — `DESIGN-STUDIO-001` (§1 layout ASCII + §8 column order), bumping
  version + Annex A; flip `FEAT-STUDIO-003` status **Open → Closed** and fill its §5 decision log;
...
re-baselined; `FEAT-STUDIO-003` status = Closed; register row = Closed.

## 7. Change-control / close-out

CR-003 is a change against the baselined `STUDIO-001` (`FEAT-STUDIO-001 §5`). At close-out the parent
clauses named in `FEAT-STUDIO-003 §3` are reconciled, the `DESIGN-STUDIO-001` version + Annex A
...
| 2026-05-29 | P3 | Re-baselined `DESIGN-STUDIO-001 §1/§8` (v0.6); `FEAT-STUDIO-003` Open → Closed; register row Closed. | ✅ |
