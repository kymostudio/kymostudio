...
6. **One owner per control:** there is **no** floating toolbar; the sample picker and the canvas
   background control live in the top bar; there is a **single** Export entry point; and the
   `Code`/`Preview` tabs reflect the code-pane state (`FR-CS-07`).
