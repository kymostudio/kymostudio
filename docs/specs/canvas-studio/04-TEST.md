...
| **TC-CS-02** | FR-CS-02 | E2E | The top bar renders. **Undo/redo:** drag a `kymo-node` → Undo restores its origin **and** the `.kymo` text round-trips (re-uses `TC-18`/`TC-J-02` behaviour); Redo re-applies. **Theme** toggle flips `[data-theme]`. **Export** downloads a board `<svg>`; **Share** writes a `?script=` link. The **Code** tab hides/shows the `.kymo` pane; **Comments**/**Versions** are present but disabled. |
| **TC-CS-03** | FR-CS-03 | E2E | Clicking an enabled rail tool sets it active **and** the canvas enters that tool (e.g. draw → a pointer-drag makes a `freedraw`); its keyboard shortcut selects the same tool; a shortcut typed **inside** the `.kymo` `<textarea>` does **not** switch tools; a disabled placeholder tool does not activate on click. |
| **TC-CS-04** | FR-CS-04 | E2E/visual | For the AIQ sample: a `kymo-node` renders with a colour stripe + glyph + title/sub; a `kymo-region` renders with a dashed (inner) / solid (outer) border + label; a `kymo-edge` shows the flow-dash animation. chrome-MCP screenshot is visually faithful to the prototype's `diagram.jsx`. |
...
|---------|------------|--------|----------------------------------|
| 0.1     | 2026-05-24 | Vũ Anh | Initial V&V: `TC-CS-01..06` (tokens/theme, top bar, tool rail, item styling, selection affordances, status bar), the regression gates (`TEST-CANVAS-001`/`TEST-JAM-001`/render-guard) + golden-safety, NFR methods, traceability. |
