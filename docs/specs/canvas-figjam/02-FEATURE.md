...
|----|-------------|-------------|-------|
| **FR-FJ-01** | The engine SHALL **consolidate the built-in shapes**: re-point `diagramToShapes.ts` from tldraw's `geo`/`arrow` to two tiny custom shapes **`kymo-region`** / **`kymo-edge`** carrying only the props kymo sets, rendered by the engine's own `ShapeUtil`s. The `patchDsl` round-trip is unaffected (it reads `meta.kymo`, not the shape type). | SN-1 | P1 |
