---
title: Canvas FigJam — Specification: Overview & Document Map
document_id: INTRO-FIGJAM-001
version: "0.1"
issue_date: 2026-05-24
status: Draft
classification: Internal
owner: diagrams/ project
audience: Anyone new to the canvas-figjam effort; engineers, reviewers
...
# Canvas FigJam — Specification: Overview & Document Map

| Field             | Value                                                              |
|-------------------|-------------------------------------------------------------------|
| Document ID       | INTRO-FIGJAM-001                                                 |
| Version           | 0.1                                                              |
| Issue Date        | 2026-05-24                                                       |
| Status            | Draft                                                           |
| Classification    | Internal                                                        |
| Owner             | `diagrams/` project                                            |
| Related Documents | `FEAT-FIGJAM-001`, `DESIGN-FIGJAM-001`, `TEST-FIGJAM-001`, `PLAN-FIGJAM-001`, `INTRO-ENGINE-001` (sibling) |

> Start here. This folder (`docs/specs/canvas-figjam/`) specifies the **second half** of the in-house
...
> plan that delivers it (phases, risks, worklog) lives in `docs/plans/canvas-figjam/`
> (`PLAN-FIGJAM-001`).
...
|-------|--------|-------------|-----------------|---------|
| **Specification** (this folder) | `docs/specs/canvas-figjam/` | Specification / Description | §6.4 Technical Processes | *what must it be / how is it built / how is it verified?* |
| **Implementation plan** | `docs/plans/canvas-figjam/` | Plan + Records — **living** | §6.3 Technical Management | *why, in what order, at what risk, what's done?* |

### 2.1 Specification layer — document map (this folder)

| # | Document | document_id | ISO/IEC/IEEE 12207 process | Answers |
|---|----------|-------------|----------------------------|---------|
| 01 | `01-INTRO.md` | `INTRO-FIGJAM-001` | 6.3.6 Information Management | *where do I start?* |
| 02 | `02-FEATURE.md` | `FEAT-FIGJAM-001` | 6.4.2 Stakeholder Needs + 6.4.3 Requirements (SRS, 29148) | *what must it do?* |
| 03 | `03-DESIGN.md` | `DESIGN-FIGJAM-001` | 6.4.4 Architecture (42010) + 6.4.5 Design Definition | *how is it built?* |
| 04 | `04-TEST.md` | `TEST-FIGJAM-001` | 6.4.9 Verification + 6.4.11 Validation + 6.3.6 Traceability | *how do we know it's right?* |

### 2.2 Implementation-plan layer (separate folder)

| Document | document_id | ISO/IEC/IEEE 12207 process | Answers |
|----------|-------------|----------------------------|---------|
| `docs/plans/canvas-figjam/PLAN.md` | `PLAN-FIGJAM-001` | 6.4.1 Mission Analysis + 6.3.1 Project Planning + 6.3.4 Risk + 6.3.2 Worklog | *why, in what order, at what risk, what's done?* |
...
- **Inherited requirements:** the engine's deferred requirements are **re-homed here** with new IDs —
  `FR-EN-06` → `FR-FJ-01` (built-in consolidation), `FR-EN-10` → `FR-FJ-02` (undo/redo), `FR-EN-11`
  → `FR-FJ-03` (board export), `NFR-EN-01/02` → `NFR-FJ-` (perf, footprint). `FEAT-FIGJAM-001` owns
...
Spec, in numeric order: **`01-INTRO`** (this doc) → **`02-FEATURE`** (the `FR-FJ`/`NFR-FJ`
requirements + the re-homing map) → **`03-DESIGN`** (consolidation, undo stack, export, removal,
footprint, freeform tools) → **`04-TEST`** (V&V, `TC-FJ-NN`, the full parity re-run, traceability).
For delivery status & history, read **`PLAN-FIGJAM-001`**.
...
- **Traceability invariant:** every requirement in `FEAT-FIGJAM-001` will have ≥ 1 covering test in
  `TEST-FIGJAM-001` before the feature is declared done.
...
|---------|------------|--------|------------------------------------------|
| 0.1     | 2026-05-24 | Vũ Anh | Initial introduction + document map for canvas-figjam (the engine-completion + FigJam-authoring half spun out of `INTRO-ENGINE-001` at the key-free-board seam). |
